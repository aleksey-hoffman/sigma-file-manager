...
import {DirEntryListDir, DirEntryListFile, DirEntryDivider, DirEntrySpacer} from '@/components/navigator/dir-entry';
...
const formattedDirEntries = computed<VirtualEntry[]>(() => {
  ...
  results.forEach(virtualEntry => {
    if (virtualEntry.type === 'dirs') {
      virtualEntry.component = 'DirEntryListDir';
    } else if (virtualEntry.type === 'files') {
      virtualEntry.component = 'DirEntryListFile';
    }
  });
