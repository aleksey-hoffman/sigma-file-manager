...
import {DirEntrySpacer} from '@/components/DirEntrySpacer';
import {DirEntryDivider} from '@/components/DirEntryDivider';
import {DirEntryListDirCard} from '@/components/DirEntryListDirCard';
import {DirEntryListFileCard} from '@/components/DirEntryListFileCard';
import {DirEntry, Divider, Spacer} from '@/types/dirEntry';
import {useI18n} from 'vue-i18n';
import {filterNavigatorView} from '@/utils/viewFilter';
import {useViewFilterStore} from '@/stores/runtime/viewFilter';
...
const navigatorVirtualListRef = ref<InstanceType<typeof VirtualScroller> | null>(null);
...
const componentReference: ComponentReferenceType = {
  ...
  DirEntrySpacer,
  DirEntryListDirCard,
  DirEntryListFileCard,
  div: 'div'
};

const formattedDirEntries = computed<VirtualEntry[]>(() => {
  ...
  results.forEach(virtualEntry => {
    if (virtualEntry.type === 'dirs') {
      virtualEntry.component = 'DirEntryListDirCard';
    } else if (virtualEntry.type === 'files') {
      virtualEntry.component = 'DirEntryListFileCard';
    }
  });
...
const bottomSpacerEntry = computed(() => (
  [{
    ...
    props: {
      ...
      onClick: () => navigatorVirtualListRef?.value?.scrollTop()
    }
  }] satisfies VirtualEntryItem[]
));
